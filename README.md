...
The agent runs two specialist sub-agents under a supervisor:

- **`brave_scout`** — broad web scan for recent news, press, and social media activity
- **`tavily_analyst`** — deep and throrough investigation on the topic

Results are scored for significance relative to previous findings, saved to a local SQLite ledger, and visualised in a Streamlit dashboard.
